...
* Outcome definition questions:
  - Measured renal dysfx: Cr >= 3 or lower?
  - Glycemic control: A1c <= 7? how frequently?
* Review of all outcome cohorts, but particularly:
  - genitourinary infection
  - joint pain
  - bone fracture
  - hypoglycemia (definition from: Comparative risk of serious hypoglycemia with oral antidiabetic monotherapy: A retrospective cohort study)
  - DKA (definition from: Diabetic ketoacidosis in patients with type 2 diabetes treated with sodium glucose co‐transporter 2 inhibitors versus other antihyperglycemic agents: An observational study of four US administrative claims databases)
...
parties <- readr::read_delim(col_names = TRUE, delim = ";", trim_ws = TRUE, file = "
  ...
  Rohan Khera; Department of Medicine, Yale University, New Haven, CT, USA
  Harlan M. Krumholz; Department of Medicine, Yale University, New Haven, CT, USA
  Yuan Lu; Department of Medicine, Yale University, New Haven, CT, USA
...
This can allow researchers and stakeholders to understand the heterogeneity of source coding for exposures and health outcomes as well as the impact of various inclusion criteria on overall cohort counts (details described in \@ref{sample-size}).

## Outcomes

Across all data sources and pairwise exposure cohorts, we will assess relative risks of **TODO #** cardiovascular and patient-centered outcomes (Table \@ref(tab:outcome-table)).
...
outcomes <- read.csv(system.file("settings/OutcomesOfInterest.csv",
                                 package = "LegendT2dm")) %>% 
  select(atlasName,description,cite,isNew) %>%
  mutate(isNew = ifelse(isNew == "yes", "yes", ""))
colnames(outcomes) <- c("Phenotype", "Brief logical description", "Supporting references", "Needs review?")
...
Within a study, for each exposure pair, we will first rebuild PS models where we additionally include baseline glucose or hemoglobin A1c measurements as patient characteristics, stratify or match patients under the new PS models that directly adjust for potential glycemic confounding and then estimate effectiveness and safety HRs.  
...
<!--
# Additional Outcomes to Evaluate {#addition-outcomes}
