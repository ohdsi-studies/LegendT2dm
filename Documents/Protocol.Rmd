...
**Study Aims**: To determine real-world comparative effectiveness and safety of second-line T2DM agents using health information encompassing millions of patients with T2DM, with a focus on individuals at moderate cardiovascular risk and other key subgroups.
...
* **Population**: Adult, T2DM patients who newly initiate a second-line T2DM agent after using metformin, including individuals with and without established cardiovascular disease 
...
LEGEND-T2DM will execute all 237 pairwise class-vs-class and drug-vs-drug comparisons in each data source that meet a minimum patient count of 1,000 per arm and extensive study diagnostics that assess reliability and generalizability through cohort balance and equipoise to examine the relative risk of cardiovascular and safety outcomes:
...
We will include all subjects in a data source who met inclusion criteria for one or more second-line T2DM agent exposure cohorts.
Broadly, these cohorts will consist of T2DM patients with prior metformin monotherapy who now initiate treatment with one of the 22 drug ingredients that comprise the DPP4 inhibitor, GLP1 receptor agonist, SGT2 inhibitor and sulfonylurea drug classes (Table \@ref(tab:drug-table)).
...
The **Class-vs-Class** Study will construct four exposure cohorts for new-users of any drug ingredient within the four second-line drug classes in Table \@ref(tab:drug-table).
...
* At least 3 months of metformin use before the index date; and
...
Three months of metformin is consistent with ADA guidelines [@American_Diabetes_Association2018-yz].
...
We will conduct important sensitivity analyses involving available HbA1c measurements to demonstrate their balance between exposure cohorts (described later in Section \@ref(section-todo)). 
...
#### TODO Add ATLAS description of sensitivity cohorts

For each data source, we will then execute all ${4 \choose 2} = 6$ pairwise class comparisons for which the data source yields $\ge$ 1,000 patients in each arm.
...
The **Drug-vs-Drug Study** will construct 22 exposure cohorts for new-users of each drug ingredient in Table \@ref(tab:drug-table).
...
For each data source, we will then execute all ${22 \choose 2} = 231$ pairwise drug comparisons.
While we will publicly report studies results for all pairwise comparisons, we will focus primary clinical interpretation and scientific publishing to the ${5 \choose 2}$ [within DPP4Is] $+ {6 \choose 2}$ [within GLPR1RAs] $+ {4 \choose 2}$ [within SGLT2Is] $+ {7 \choose 2}$ [within SUs] $= 52$ comparisons that pit drugs within the same class against each other, as well as across-class comparisons that stakeholders deem pertinent given their experiences.
...
2. **TODO** alternative cohort exit for treatment escalation (OT2)
...
baseCohortJson <- SqlRender::readSql(system.file("cohorts", "ID100000.json", package = "LegendT2dm"))
baseCohort <- RJSONIO::fromJSON(baseCohortJson)

baseCohort$ConceptSets <- baseCohort$ConceptSets[-c(8, 9, 13)] # remove CVD and renal
baseCohortJson <- RJSONIO::toJSON(baseCohort, digits = 50)

printCohortDefinitionFromNameAndJson(name = "Class-vs-Class Exposure (DPP4 New-User) Cohort",
                                     json = baseCohortJson)
```
...
# Age
youngerJson <- SqlRender::readSql(system.file("cohorts", "ID110000.json", package = "LegendT2dm"))
youngerCohort <- CirceR::cohortExpressionFromJson(youngerJson)
printInclusionCriteria(youngerCohort)

middleJson <- SqlRender::readSql(system.file("cohorts", "ID120000.json", package = "LegendT2dm"))
middleCohort <- CirceR::cohortExpressionFromJson(middleJson)
printInclusionCriteria(middleCohort)

olderJson <- SqlRender::readSql(system.file("cohorts", "ID130000.json", package = "LegendT2dm"))
...
# Sex
femaleJson <- SqlRender::readSql(system.file("cohorts", "ID101000.json", package = "LegendT2dm"))
femaleCohort <- CirceR::cohortExpressionFromJson(femaleJson)
printInclusionCriteria(femaleCohort)

maleJson <- SqlRender::readSql(system.file("cohorts", "ID102000.json", package = "LegendT2dm"))
...
# Race
blackJson <- SqlRender::readSql(system.file("cohorts", "ID100100.json", package = "LegendT2dm"))
...
# CVD
lowCvdJson <- SqlRender::readSql(system.file("cohorts", "ID100010.json", package = "LegendT2dm"))
lowCvdCohort <- CirceR::cohortExpressionFromJson(lowCvdJson)

printInclusionCriteria(lowCvdCohort)

highCvdJson <- SqlRender::readSql(system.file("cohorts", "ID100020.json", package = "LegendT2dm"))
...
# Renal
noRenalJson <- SqlRender::readSql(system.file("cohorts", "ID100001.json", package = "LegendT2dm"))
noRenalCohort <- CirceR::cohortExpressionFromJson(noRenalJson)

printInclusionCriteria(noRenalCohort)

withRenalJson <- SqlRender::readSql(system.file("cohorts", "ID100002.json", package = "LegendT2dm"))
