...
## Configuration files
  ...
  * `baseCohort.json`
  * `extraGenerateExposureCohortDefinitions.R`
  * `inst/settings/classCohortsToCreate.csv`
    - `atlasId`, `atlasName`, `cohortId`, `name`
  * `inst/settings/classTcodOfInterest.csv`
    - `targetId`, `comparatorId`, `outcomeIds`, `excludedCovariateConceptIds` ,`targetName`, `comparatorName`
  * `inst/settings/ExposuresOfInterest.csv`
    - `type`, `name`, `cohortId`, `conceptId`, `class`, `shortName`, `order`, ` `includedConceptIds`
  * `inst/settings/Indications.csv`
    - `indicationId`, `indicationName`, `filterConceptIds` , `positiveControlIdOffset`
  * `inst/settings/OutcomesOfInterest.csv`
    - `cohortId`, `atlasId`, `atlasName`, `name`, `description`, `cite`, `isNew`
